    ...
    stages {
        ...
        stage('CD - Build Target') {
            steps {
                sshagent(credentials: ['studion-ec2-ssh']) {
                    sh '''
                        ssh ${DEPLOY_USER}@${DEPLOY_HOST} 'cd '"${DEPLOY_DIR}"' && docker compose --env-file .env.prod -f compose.prod.yaml up -d redis mysql mongodb && for c in redis mysql mongodb; do i=0; while [ "$i" -lt 30 ]; do [ "$(docker inspect -f "{{.State.Health.Status}}" "$c" 2>/dev/null)" = healthy ] && break; i=$((i + 1)); sleep 5; done; [ "$(docker inspect -f "{{.State.Health.Status}}" "$c")" = healthy ] || exit 1; done && docker compose --env-file .env.prod -f compose.'"${TARGET_COLOR}"'.yaml build'
                    '''
                }
            }
        }
        ...
        stage('CD - Health Check') {
            steps {
                sshagent(credentials: ['studion-ec2-ssh']) {
                    sh '''
                        ssh ${DEPLOY_USER}@${DEPLOY_HOST} 'port='"${TARGET_BACKEND_PORT}"'; container=studion-backend-'"${TARGET_COLOR}"'; for i in $(seq 1 30); do curl -fsS http://127.0.0.1:${port}/actuator/health | grep -q UP && exit 0; sleep 5; done; docker ps -a --filter "name=${container}" --filter "name=mysql" --filter "name=mongodb" --filter "name=redis"; docker logs --tail=200 "${container}" || true; docker logs --tail=80 mysql || true; docker logs --tail=80 mongodb || true; exit 1'
                    '''
                }
            }
        }
